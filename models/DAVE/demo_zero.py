    ...
    def one_img(img_path):
        ...
        for i in range(len(pred_boxes)):
            ...
            plt.plot([box[0], box[0], box[2], box[2], box[0]], [box[1], box[3], box[3], box[1], box[1]], linewidth=2,
                    color='red')
        # Aggiunto per fare prova resizing di density map
        orig_num_obj = denisty_map.sum().item()
        resize_den = T.Resize((648, 648), interpolation=T.InterpolationMode.NEAREST)
        den_map_resized = T.Compose([
            resize_den,
        ])(denisty_map)
        if den_map_resized.sum() != 0:
            den_map_resized = orig_num_obj*den_map_resized/den_map_resized.sum()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path', type=Path, default="output")
