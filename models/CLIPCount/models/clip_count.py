...
from ..util.constant import EOS_TOKEN_ID
import clip
...
class CLIPCount(nn.Module):
    def __init__(self, fim_depth:int=4, 
        ...
        self.text_encoder = CLIPTextTransformer(self.clip, use_coop=self.use_coop, n_ctx = self.coop_width, use_post_ctx=False)
...
class CLIPTextTransformer(nn.Module):
    ...
    """
    def __init__(self, clip_model, use_coop:bool, n_ctx:int = 2, use_post_ctx:bool = True) -> None:
        super().__init__()
        ...
        if use_coop:
            ...
            self.use_post_ctx = use_post_ctx
            if self.use_post_ctx:
                post_context_vectors = torch.empty(self.n_ctx, self.clip_model.ln_final.weight.shape[0])
                torch.nn.init.normal_(post_context_vectors, std=.02)
                self.post_learnable_context = nn.Parameter(post_context_vectors) # post prompt context
    ...
    def forward(self, text):
        ...
        if self.use_coop:
            ...
            if self.use_post_ctx is False:
                ...
                eos_idx = (text == EOS_TOKEN_ID).nonzero(as_tuple=False)[:, 1:2].T[0].tolist() #indexes of eos in each tokenized text, shape = [batch_size, 1]
                
                sos_token = x[:, 0, :].unsqueeze(1)  # [batch_size, 1, d_model]  
                
                pre_ctx = einops.repeat(self.learnable_context, 'n d -> b n d', b=x.shape[0])      
                post_ctx = einops.repeat(self.post_learnable_context, 'n d -> b n d', b=x.shape[0])  
                
                concatenated_tensors = []
                
                for i in range(x.shape[0]):
                    # split x into two parts based on the specified index for each row
                    x1 = x[i, 0:eos_idx[i], :]
                    x2 = x[i, eos_idx[i]:, :]
                    concatenated_row = torch.cat((x1.unsqueeze(0), post_ctx[i].unsqueeze(0), x2.unsqueeze(0)), dim=1)
                    concatenated_tensors.append(concatenated_row)

                prompt_tokens = torch.cat(concatenated_tensors, dim=0)    
                prompt_tokens = prompt_tokens[:, 1:-self.n_ctx*2, :]
                
                '''sos_token = x[:, 0, :].unsqueeze(1)  # [batch_size, 1, d_model]
                prompt_tokens = x[:, 1:-self.n_ctx*2, :] # class tokens + [EOS] token
                pre_ctx = einops.repeat(self.learnable_context, 'n d -> b n d', b=x.shape[0])
                post_ctx = einops.repeat(self.post_learnable_context, 'n d -> b n d', b=x.shape[0])
                x = torch.cat([sos_token, pre_ctx, prompt_tokens, post_ctx], dim=1)'''
                
                x = torch.cat([sos_token, pre_ctx, prompt_tokens], dim=1)
