...
class FSC147(Dataset):
    def __init__(self, 
                 ...
                 additional_prompt:bool=False,
                 prompt_with_captioning:bool=False):
        """
        Parameters
        ----------
        ...
        self.captioning_module_dir = "captioning-module/"
        ...
        self.captions_file = os.path.join(self.captioning_module_dir, f'generated_captions_{self.dataset_type}.json')
        self.promptcap_file = os.path.join(self.captioning_module_dir, f'promptcap_vqa_generated_captions_{self.dataset_type}.json')
        self.split = split
        ...
        self.captions = {}  # initialize self.captions as an empty dictionary

        with open(self.captions_file) as f:
            for caption in json.load(f):
                img_name = caption["img_name"]
                generated_caption = caption["generated_caption"]
                self.captions[img_name] = generated_caption
        
        self.promptcap_captions = {}
        
        with open(self.promptcap_file) as f:
            for caption in json.load(f):
                img_name = caption["img_name"]
                generated_caption = caption["generated_caption"]
                self.promptcap_captions[img_name] = generated_caption
        ...
        # option to activate prompt with captioning
        self.use_prompt_with_captioning = prompt_with_captioning
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train' or (self.split == 'val' and self.resize_val):
            ...
            if self.use_prompt_with_captioning:
                
                # retrieve image caption from file    
                generated_caption = self.captions[im_id]
                text_with_caption = f"{text} in a photo that has been captioned as follows {generated_caption}"
                class_name = text
                
                prompt_cap = self.promptcap_captions[im_id]
                
                return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], prompt_cap, class_name, im_id
            ...
            if self.use_prompt_with_captioning:
                ...
                return sample['image'].float(), sample['gt_map'], sample['boxes'], sample['pos'], prompt_cap, class_name, im_id
